...
const threeId = pokémon.filter( p => p.id % 3 === 0 );
...
const nameList = pokémon.map(p => p.name);

// an array with just the names of pokémon with an id greater than 99
const ninetyninePkmn = pokémon.filter( p => p.id > 99).map( p => p.name);

// an array with just the names of the pokémon whose only type is poison
const poison = pokémon.filter( p => p.types == "poison" && p.types.length == 1).map( p => p.name);
